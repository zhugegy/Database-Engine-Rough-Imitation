...
public:
  ...
  int set_array_element_count(int nNum);
...
template<typename T>
int CGYDynamicArray<T>::set_array_element_count(int nNum)
{
  m_nArrayElementCount = nNum;
...
bool CGYDynamicArray<T>::delete_element(int nIndex)
{
  ...
  for (int i = nIndex; i < m_nArrayElementCount - 1; i++)
  {
...
template<typename T>
bool CGYDynamicArray<T>::empty_array()
{
  while (get_array_element_count() != 0)
  {
    delete_element(0);
  }
